...
class Common_SSL_Tests(unittest.TestCase):
    ...
    def test_new(self):
        self.assertTrue(self._SSL_CLS(self._SSL_CTX_CLS(OpenSslVersionEnum.SSLV23.value)))

    def test_new_bad(self):
        # Invalid None SSL_CTX
        self.assertRaises(TypeError, self._SSL_CLS, (None))

    def test_set_verify(self):
        test_ssl = self._SSL_CLS(self._SSL_CTX_CLS(OpenSslVersionEnum.SSLV23.value))
    ...
    def test_set_session_bad(self):
        test_ssl = self._SSL_CLS(self._SSL_CTX_CLS(OpenSslVersionEnum.SSLV23.value))
        self.assertRaisesRegexp(TypeError, 'must be self._SSL_CLS_SESSION', test_ssl.set_session, None)
...
class Legacy_SSL_Tests(Common_SSL_Tests):
    ...
    _SSL_CLS = _nassl_legacy.SSL
    _SSL_CTX_CLS = _nassl_legacy.SSL_CTX
    _BIO_CLS = _nassl_legacy.BIO
