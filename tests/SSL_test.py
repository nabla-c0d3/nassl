...
from nassl.base_ssl_client import BaseSslClient, TlsVersionEnum, OpenSslVerifyEnum
...
class TestCommonSSL:
    ...
    def test_new_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
    ...
    def test_set_bio(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_bio = nassl_module.BIO()
        test_ssl.set_bio(test_bio)

    def test_set_bio_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        # Invalid None BIO
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        with pytest.raises(TypeError):
            test_ssl.set_bio(None)

    def test_set_connect_state(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.set_connect_state()

    # Can't really unittest a full handshake, read or write
    def test_do_handshake_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        # Connection type not set
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        with pytest.raises(OpenSSLError, match="connection type not set"):
            test_ssl.do_handshake()

    def test_pending(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
    ...
    def test_get_current_compression_method(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
    ...
    def test_set_tlsext_host_name(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.set_tlsext_host_name("tests")

    def test_set_tlsext_host_name_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        with pytest.raises(TypeError):
            test_ssl.set_tlsext_host_name(None)

    def test_set_cipher_list(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.set_cipher_list("HIGH")

    def test_shutdown_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        with pytest.raises(OpenSSLError, match="uninitialized"):
            test_ssl.shutdown()

    def test_get_cipher_list(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        assert test_ssl.get_cipher_list()

    def test_get_cipher_name(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.get_cipher_name()

    def test_get_cipher_bits(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        assert 0 == test_ssl.get_cipher_bits()

    def test_get_client_CA_list_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        assert [] == test_ssl.get_client_CA_list()

    def test_get_verify_result(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        assert 0 == test_ssl.get_verify_result()

    def test_renegotiate(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.renegotiate()

    def test_get_session(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.get_session()

    def test_set_session_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        with pytest.raises(TypeError):
            test_ssl.set_session(None)

    def test_set_options_bad(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        assert 0 <= test_ssl.set_options(123)

    def test_set_tlsext_status_type(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
        test_ssl.set_tlsext_status_type(BaseSslClient._TLSEXT_STATUSTYPE_ocsp)

    def test_get_tlsext_status_type(self, nassl_module: ModuleType, SSL_CTX_args: list[TlsVersionEnum]) -> None:
        test_ssl = nassl_module.SSL(nassl_module.SSL_CTX(*SSL_CTX_args))
...
class TestSSL_OpenSSL_1_0_2:
    ...
    def test_set_cipher_list_bad(self) -> None:
        # Invalid cipher string
        test_ssl = nassl.openssl_1_0_2._nassl.SSL(_SSL_CTX_OpenSSL_1_0_2(TlsVersionEnum.TLS_1_2.value))
